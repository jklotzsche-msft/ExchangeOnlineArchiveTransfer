Describe "Validating the module manifest" {
	$moduleRoot = (Resolve-Path "$global:testroot\..\ExchangeOnlineArchiveTransfer").Path
	$manifest = ((Get-Content "$moduleRoot\ExchangeOnlineArchiveTransfer.psd1") -join "`n") | Invoke-Expression
